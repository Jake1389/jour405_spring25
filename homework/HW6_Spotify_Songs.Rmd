...
spotify_songs |> 
  ...
  geom_vline(aes(xintercept = mean(danceability)), color="red", linetype = "dashed", size = 1) + geom_vline(aes(xintercept = mean(danceability) - sd(danceability)), color="blue", linetype = "dashed", size = 1) + geom_vline(aes(xintercept = mean(danceability) + sd(danceability)), color="blue", linetype = "dashed", size = 1)
```
...
```{r}
I added it all in the same task, like we did in class.
```
...
```{r}
I did it all in the same task, like we did in class.
...
The curve is skewed a bit to the right, as the count immediately right to the mean is very high. The mean and standard deviation prove this point, as the mean has the highest count and the count immediately to the right is also large. My pitch would be about the range of danceability from 0.67 to 0.85 — the counts are high there so I think that would make for an interesting story. Why are the counts high in those places specifically? What does this tell about the music? Etc. 
