...
This seems like a useful model since it provides multiple ways to gauge significance of various statistics. For example, I was able to see through the coefficients that yards gained isn't very important, while points scored is and takeaways might be. The R-squared value is also pretty high, telling that there is some sort of significance here. The F-statistic corroborates this as it shows a statistical significance. However, there are limits within this model as it doesn't exactly tell anything groundbreaking. I can see that there's a significance in points scored to wins, but that's pretty obvious anyway. Overall the model helps narrow the scope a bit, but it doesn't exactly provide insight on various stats on it's own. But at least now I know that takeaways, for example, may be something to focus on — it could have a significance, it's just not totally clear here.
...
Points scored has the most impact on wins, which was pretty expected going in. Yards gained seems to have a very minimal impact on wins, which is definitely interesting. It makes sense as things like special teams, turnovers, poor red-zone offense and such can all conribute to teams either succeeding or failing regardless of yards gained.
...
I could use this to think about what to analyze for a game story. For example, say it's week 5 of the NFL season and thus far in the season there has been a large correlation between turnovers forced and wins. Imagine I'm writing about a game and there are turnovers forced that are the primary factor in deciding it — that could be my primary angle for the game story, sort of connecting it to the broader trend. Overall, I think this model could be useful as it could help find these trends, which may or may not be especially useful (it just depends, which is why it's crucial to test things out). Even over an entire season, a reporter could analyze trends and how they change to potentially find angles for stories. 
