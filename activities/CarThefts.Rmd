...
car_thefts |> 
  ggplot() + 
  geom_histogram(aes(x = count), binwidth = 2) + geom_vline(aes(xintercept = mean(count)), color="red", linetype = "dashed", size = 1) + geom_vline(aes(xintercept = mean(count) - sd(count)), color="blue", linetype = "dashed", size = 1) + geom_vline(aes(xintercept = mean(count) + sd(count)), color="blue", linetype = "dashed", size = 1)
```
...
The bell curve is skewed to the right a bit, with the range of 8-16 cars a day being stolen the most common -- especially the higher end of that as the 12-16 range has the most occurrences. 
